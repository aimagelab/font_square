...
class Render:
    # ink_path = None  # Used only by the function that seems to never be called
    # pag_path = None

    def __init__(self, font_path, height=None, width=None, font_size=64, calibrate=False, calib_text=None, calib_threshold=0.7, calib_h=128):
        self.font_path = font_path
        self.font_size = font_size
        self.height = height if height is not None else calib_h
        self.width = width

        self.font = ImageFont.truetype(self.font_path, self.font_size)
        if calibrate:  # Compute the font size to keep the text at calib_threshold% of the height
            self._font_calibration(calib_text, calib_threshold, calib_h)
            self.font = ImageFont.truetype(font_path, self.font_size)
    
    def _font_calibration(self, text=None, threshold=0.85, height=128, width=2500):
        text = string.ascii_letters if text is None else text
        while self._perc(text, height, width) > threshold:
            self.font_size -= 1
            if self.font_size <= 1: break
            self.font = ImageFont.truetype(self.font_path, self.font_size)
            print(f'  Calibrating {self.font_path.ljust(50)} font_size: {self.font_size} '.ljust(terminal_columns), end='\r')
        while self._perc(text, height, width) < threshold:
            self.font_size += 1
            self.font = ImageFont.truetype(self.font_path, self.font_size)
            print(f'  Calibrating {self.font_path.ljust(50)} font_size: {self.font_size} '.ljust(terminal_columns), end='\r')
        print(f'  Calibrating {self.font_path.ljust(50)} font_size: {self.font_size} COMPLETE'.ljust(terminal_columns))

        return self.font_size
    
    # def set_templates(self, pag_path, ink_path):  # NU
    ...
    def render(self, text, return_np=False, action='random', pad=0):
        ...
        h = self.height
        w = self.width if self.width is not None else bbox_width + 2 * pad
        ...
        # img = self._convert_colors(img)  # FIXME seems to be a call that never does anything
